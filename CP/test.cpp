...
/*void dfs(vector<vli> &graph,lli node,lli par,vli &pr,vli &ans,bool &flag){
	if(par != -1){
		if(pr[node]<pr[par]){
			flag = true;
			return;
		}else{
			ans[node] = pr[node] - pr[par];
		}
	}
	for(auto child : graph[node]){
		if(child != par){
			dfs(graph,child,node,pr,ans,flag);
			if(flag){
				return;
			}
		}
...
lli xorSum(vli arr, lli n)
{
    lli bits = 0;
    for (lli i=0; i < n; ++i)
        bits |= arr[i];
    lli ans = ((bits%mods) * (binpow(2, n-1)%mods))%mods;
    return ans;
}

void buildtree(vli &seg,vli &arr,lli id,lli l,lli r){
    if(l==r){
        seg[id]=arr[l];
        return;
    }
    lli mid=(l+r)/2;
    buildtree(seg,arr,2*id,l,mid);
    buildtree(seg,arr,2*id+1,mid+1,r);
...
int main()
{
    ...
    while(t--){ 
        lli q1,q2;
        cin>>q1>>q2;
        cout<<query(segtree,1,0,n-1,q1,q2)<<endl;
    }
    return 0;
}   
